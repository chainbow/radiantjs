...
Transaction.MAX_MONEY = '2100000000000000000'
...
Transaction.prototype._getHash = function () {
  if (this.version === 3) {
    return this.getHashV3()
  }
  return Hash.sha256sha256(this.toBuffer())
}

Transaction.prototype.getHashV3 = function () {
  if (this.version !== 3) {
    throw new Error('Must be v3')
  }

  const txHashWriter = new BufferWriter()
  // Write version
  let buf = Buffer.allocUnsafe(4)
  buf.writeUInt32LE(this.version)
  txHashWriter.write(buf)

  // Write inputs length
  buf = Buffer.allocUnsafe(4)
  buf.writeUInt32LE(this.inputs.length)
  txHashWriter.write(buf)

  // Write hash prevout inputs
  txHashWriter.write(Sighash.GetHashPrevoutInputs(this))

  // Write hash prevout inputs
  txHashWriter.write(Sighash.GetSequenceHash(this))

  // Write outputs length
  buf = Buffer.allocUnsafe(4)
  buf.writeUInt32LE(this.outputs.length)
  txHashWriter.write(buf)

  // Write hash output hashes
  txHashWriter.write(Sighash.GetHashOutputHashes(this))
  ...
  return Hash.sha256sha256(txHashWriter.toBuffer())
}
