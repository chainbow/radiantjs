 ...
 * @private
 */
Block._fromObject = function _fromObject ({ header, transactions }) {
  if (!header || !transactions) throw new TypeError('Invalid arguments for Block')
  return {
    header: BlockHeader.fromObject(header),
    transactions: transactions.map(tx => tx instanceof Transaction ? tx : Transaction().fromObject(tx))
  }
}
...
Block._fromBufferReader = function _fromBufferReader (br) {
  ...
  return {
    header: BlockHeader.fromBufferReader(br),
    transactions: [...Array(br.readVarintNum()).keys()].map(() => Transaction().fromBufferReader(br))
  }
}
 ...
 * @returns {Block} - An instance of block
 */
Block.fromBufferReader = function fromBufferReader (buf) {
  $.checkArgument(buf, 'buf is required')
  return new Block(Block._fromBufferReader(buf))
}
